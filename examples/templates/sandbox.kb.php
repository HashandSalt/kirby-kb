...
<article>
  ...
  <!-- KB:EMAIL -->
  <div class="kb-example ">
    <p><kb:email address="email@hashandsalt.com" text="Email me" class="email-link" /></p>
    <p><kb:email field="postauthor" text="Email the author" class="email-link" /></p>
  </div>

  <!-- KB:TEL -->
  <div class="kb-example ">
    <p><kb:tel number="+1234567890" text="Call me" class="tel-link" /></p>
    <p><kb:tel field="telnumber" text="Call me" class="tel-link" /></p>
  </div>

  <!-- KB:DATE -->
  <div class="kb-example">
    <p><kb:date field="postdate" format="d M, Y">Published on</kb:date></p>
    <p> Copyright &copy; 2021 - <kb:date format="Y" /></p>
  </div>
  ...
  <!-- KB:A -->
  <kb:a url="{{ $page->url() }}" ><kb:title /></kb:a>
  <kb:a url="$page->url()"><kb:title/></kb:a>
  <kb:a url="$somevar"><kb:title/></kb:a>
  ...
  <!-- short version -->
  <kb:img src="team.jpg" mode="crop" quality="80" format="webp" object-fit="cover" ratio="16/9" width="1440" height="450" class="team-image" />
  ...
  <kb:image src="$pageimg" mode="crop" quality="80" format="webp" object-fit="contain" ratio="16/9" width="1440" height="450" class="team-image" alt="$pageimg->alt()->esc()" />

  <!-- fallback image if no page image is set -->
  <kb:if condition="!$pageimg">
    <kb:image src="team.jpg" mode="crop" quality="80" format="webp" object-fit="contain" ratio="16/9" width="1440" height="450" class="fallback-image" alt="Fallback image" />
  </kb:if>
  ...
  <kb:foreach items="$site->children()->listed()" as="item">
    <li>
      <kb:if condition="$item->isOpen()">
        <kb:a aria-current="page" url="{{ $item->url() }}"><kb:title page="$item"/></kb:a>
      <kb:else />
        <kb:a url="{{ $item->url() }}"><kb:title page="$item"/></kb:a>
      </kb:if>
    </li>
  </kb:foreach>
  </ul>
  ...
  <!-- KB:STRUCTURE -->
  <kb:structure field="social" as="platform" wraptag="ul" breaktag="li" class="contact-social" breakclass="contact-social-item">
    <kb:a url="{{ $platform->url()->esc() }}" target="_blank" rel="noopener noreferrer">
     <kb:field name="$platform->platform()" />
    </kb:a>
  </kb:structure>
